...
**`career` は仮置きのまま（`要記入`）**。実際の期間と所属に差し替えること。

`skillGroups` の各項目は `years` と `level` を省略するとタグ表示になる。
両方入れるとメーター表示に切り替わる。根拠のない数字を並べないため、既定では省略している。
...
モバイルでは装飾（キービジュアル、ポートレート枠、作品の説明文、キャプション）を隠す。
**Email と SNS リンクは隠さない** — ここを落とすと連絡手段が消えるため。

1画面に収まりきらず、パネル内部のスクロールが出るのは以下だけ:

| 画面         | セクション                       |
| ------------ | -------------------------------- |
| 390×844 以下 | Works                            |
| 360×640 以下 | Works / Skills / Contact         |
| 320×568 以下 | Works / About / Skills / Contact |

`1280×600` のような縦の短いデスクトップでは、全セクションが1画面に収まる。
スクロールが要る場合だけ、パネル本体に `tabindex="0"` が付いてキーボードで掴めるようになる。
