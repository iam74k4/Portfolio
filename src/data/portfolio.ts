 ...
 *
 * 文言の書き方。層ごとに語尾の型を決めてある。層をまたいで混ぜないこと:
 * - 見出し（headline / contact.title）は言い切り。動詞で終える
 * - 本文（lead / bio / contact.lead）は「です・ます」
 * - カードの説明（works の summary）は「何であるか。何をしたか。」の2文。
 *   1文目は体言止め、2文目は「である」（〜している / 〜した）
 * - ラベル（career / skillGroups / role / metric.note）は名詞のまま。文にしない
...
export const profile = {
  ...
  /*
   * Home は「名乗り + 一言 + 行き先」だけに絞る。
   * 実績や所属を足したくなるが、それらは About と Works にある。
   * 同じことを2回言わせないこと。
   */
  /** Home の大見出し。行ごとに配列で持つ */
  headline: ['つくる速さを、', '仕組みで変える。'],
  lead: '金融系の基幹システムを開発しながら、生成AIと自動化で「つくる工程そのもの」を速くすることに取り組んでいます。',
  bio: [
    ...
    '生成AIと自動化ツールで開発工程を効率化することに取り組んでいます。設計書の Markdown 変換、画面コードの生成、テスト支援などを通じて、40人日を見込んでいた14画面の製造・単体テストを約20人日で完了しました。製造業の案件では、ドキュメントを参照して問い合わせに答えるエージェントを Copilot Studio で構築しています。ステップ数計算やテスト件数集計など、プロジェクトを横断して使えるツールの作成・展開も行っています。',
  ...
  contact: {
    /* Home の見出しと同じく言い切りでそろえる。体言止めにすると直下の本文と型がぶつかる */
    title: ['AI 活用の話も、', 'コードの話もしたい。'],
...
export const works: Work[] = [
  {
    title: '開発工程の効率化',
    /*
     * 人日の数字は下の metric が出すので、説明文では繰り返さない。
     * このカードだけ metric のぶん背が高く、説明が1行削られやすい。短くしておく。
     */
    summary:
      '生成AIと自動化を設計・製造・テストに組み込む取り組み。横断で使える集計ツールも展開している。',
    year: '2024 —',
    role: '業務 / 金融系基幹システム',
  ...
  },
  {
    ...
    summary:
      'Cursor / Claude Code / Copilot でばらつくルールを1か所に集約し、1コマンドで各環境へ配る設定ツール。本業の AI 活用を自分の環境にも通すためにつくった。',
    year: '2026',
    role: '個人開発',
